...
const Instruction = () => {
    return (
        <section className="w-full flex justify-center mb-6 overflow-hidden">
            {/* Lưới Bento Editorial Grid Chính với Stagger Animation lặp lại */}
            <motion.div
            ...
            >
                {/* TRÁI: Ô chứa Headline hầm hố */}
                <motion.div variants={blockReveal} className="flex flex-col justify-end bg-surface-container-high rounded-[2.5rem] p-8 md:p-14 lg:p-16 relative overflow-hidden group min-h-[400px] md:min-h-[500px] lg:min-h-[500px]">
                    {/* Hình nền nghệ thuật ẩn chìm */}
                    <div className="absolute inset-0 w-full h-full z-0">
                        ...
                        />
                        {/* Làm dịu bớt ảnh để nổi chữ lên trên */}
                    ...
                    <div className="relative z-10 w-full">
                        {/* Sub-tag thanh lịch */}
                        <motion.div variants={textFadeUp} className="inline-flex items-center gap-4 mb-6 md:mb-10">
                        ...
                        {/* Mixed Typography (Sans thanh thoát x Serif Italic) */}
                        <motion.h1 variants={textFadeUp} className="text-[4.5rem] md:text-[6rem] lg:text-[7.5rem] leading-[0.9] text-zinc-900 mb-6 md:mb-10">
                        ...
                        <motion.p variants={textFadeUp} className="max-w-md text-zinc-600 text-base md:text-lg leading-relaxed italic font-light">
                            "Góc nhỏ của những điều xinh đẹp." ✨
                        </motion.p>
                    </div>
                </motion.div>

                {/* PHẢI: Lưới dọc chứa Ảnh và Social */}
                <div className="flex flex-col gap-4 md:gap-6">

                    {/* TRÊN: Ô Ảnh Macro chi tiết (Vật liệu, chất lỏng) */}
                    <motion.div variants={blockReveal} className="flex-1 rounded-[2.5rem] overflow-hidden relative group min-h-[300px] md:min-h-[400px] bg-rose-50 border border-zinc-100">
                        <img
                        ...
                        />
                        {/* Overlay che bóng nhẹ */}
                        <div className="absolute inset-0 bg-black/5 group-hover:bg-transparent transition-colors duration-[1.5s]"></div>

                        {/* Đánh dấu chương/phần nhỏ ở góc */}
                        <div className="absolute top-6 left-8 md:top-8 md:left-10 text-zinc-900/40 font-serif italic text-3xl font-light">
                            01.
                        </div>
                    </motion.div>

                    {/* DƯỚI: Ô mạng xã hội và Khám phá lồng ghép (Thiết kế trong suốt) */}
                    <motion.div variants={blockReveal} className="h-[150px] md:h-[220px] rounded-[2.5rem] flex flex-row justify-evenly items-center text-center px-4 relative">

                        {/* Khối TikTok (Thiết kế cũ) */}
